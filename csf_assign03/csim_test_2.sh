...
# Define write-allocate policies
write_allocate_policies=("write-allocate" "no-write-allocate")

# Define write-through/back policies
...
# Output file
output_file="cache_simulations_output_3.txt"
    ...
    for set_size in "${set_sizes[@]}"; do
        for block_per_set in "${blocks_per_set[@]}"; do
            for eviction_policy in "${eviction_policies[@]}"; do
                for write_allocate_policy in "${write_allocate_policies[@]}"; do
                   for write_through_back_policy in "${write_through_back_policies[@]}"; do 

                        if ([ "$write_allocate_policy" == "no-write-allocate" ] && [ "$write_through_back_policy" == "write-back" ]); then
                            continue
                        fi
                        # Run the simulation and capture the output
                        simulation_output=$(./csim "$set_size" "$block_per_set" "16" "$write_allocate_policy" "$write_through_back_policy" "$eviction_policy" < "$trace")
                        ...
                        config_str="sets=$set_size, blocks=$block_per_set, evic_policy=$eviction_policy, write_alloc=$write_allocate_policy, write_thru=$write_through_back_policy, trace=$trace"
