...
public:
    ...
    Slot* getSlot(uint32_t tag, uint64_t globalCounter); // Retrieve a pointer to a slot based on the tag
    bool addSlot(uint32_t tag, bool lru, bool fifo, uint64_t globalCounter); // Add a slot with the provided tag
